`Solidity compiler`モジュールに切り替えます。
...
これは基本的にコンパイラが返す内容です：
 - `linkReferences`は、コントラクトが使用するライブラリを記述します。
 - `object`はコンパイルされたコントラクト（バイトコード）です。これがブロックチェーンにデプロイされ保存されます。この例では、バイトコード内の`__$d42d70ba92b626965f4c69b39148e37a33$__`の値はライブラリアドレスのプレースホルダーです。あなたの場合、プレースホルダーは`__$`と`$__`の間にあります。

Remix IDEのメタデータJSONは、このプレースホルダーを指定されたアドレスで置き換えるようにRemixに指示します。
